...
# 3. تحميل البيانات مع معالجة الأخطاء والـ NaN
@st.cache_data
def load_data():
    file_path = "data.xlsx"
    if os.path.exists(file_path):
        try:
            ...
            if 'المعدل' in df.columns:
                # حل مشكلة تحويل القيم غير المنتهية أو الفارغة إلى 정수
        ...
        if st.button("استعلام"):
            if search_query:
                ...
                if not result.empty:
                    ...
                    if "ناجح" in status:
                        st.success(f"🎉 مبارك النجاح! {student['الاسم']}") # تأكد من اسم العمود 'الاسم' أو 'اسم'
                        st.balloons()
                    else:
                    ...
                    with st.expander("🔍 تفاصيل المواد (الأحمر يعني رسوب)"):
                        for s in subjects:
                            grade = student[s]
                            # فحص إذا كانت الدرجة رقمية وأقل من 50
                            try:
                                if float(grade) < 50:
                                ...
                                st.write(f"**{s}:** {grade}") # في حال كانت القيمة نصية
                else:
